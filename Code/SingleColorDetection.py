...
def colorLimit(lowerLimit, upperLimit):
    lower = np.array(lowerLimit,np.uint8)
    upper = np.array(upperLimit,np.uint8)
    return lower, upper
...
#Defining the Range of Red color (BGR)
            red_lower, red_upper = colorLimit([136,87,111], [180,255,255])
